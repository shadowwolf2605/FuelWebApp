...
export function Card({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <div
      className={`bg-white dark:bg-[#1e1e22] rounded-2xl border border-black/5 dark:border-white/[0.07] card-shadow ${className}`}
    >
      {children}
    </div>
  );
...
export function Divider() {
  return <div className="h-px bg-gray-100 dark:bg-[#2a2a2e]" />;
}
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          {unit && (
            <span className="text-[13px] text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-[#2c2c30] px-2 py-[3px] rounded-md flex-shrink-0">
              {unit}
            </span>
          )}
        </div>
...
export function StatPill({ icon, colorClass, label, value, unit }: {
  ...
  return (
    <div className="flex-1 bg-gray-50 dark:bg-[#28282c] rounded-xl p-3 flex flex-col gap-1">
  ...
  return (
    <motion.button
      ...
      className={`w-full flex items-center justify-center gap-2 py-3.5 rounded-xl text-[15px] font-semibold transition-all duration-200 shadow-sm ${
        disabled
          ? "bg-gray-100 dark:bg-[#28282c] text-gray-300 dark:text-gray-600 cursor-not-allowed shadow-none"
          : colors[color]
      }`}
    >
      {children}
